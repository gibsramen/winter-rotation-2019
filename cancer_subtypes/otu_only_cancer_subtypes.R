...
t.norm.cancer_otu_df <- data.normalization(t.cancer_otu_df,
...
for (i in c(2:max_num)){
	...
	out_dir = paste('otu_cluster_results', cancer_type, 
					sprintf('%i_clusters', i), sep='/')
	dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)
	result <- ExecuteCC(clusterNum = i, d=t.norm.cancer_otu_df,
...
sil_out_file = paste('otu_cluster_results', cancer_type, 'silhouette_widths.tsv',  sep='/')
